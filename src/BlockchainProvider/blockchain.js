...
const Blockchain = (props) => {
  ...
  const initPersonalData = async () => {
    ...
    let holdAmount = beautifyNumber(
      (await contract.methods.getHoldAmount(account).call()).amount,
      true
    );
    let releasedAmount = beautifyNumber(
      await contract.releasedAmount(account).call(),
      true
    );
    let contractBalance = beautifyNumber(
      await contract.methods.getContractBalance().call(),
      true
    );
    let totalUsers = (await contract.totalUsers().call()).toNumber();
    let admin1Wallet = beautifyNumber(
    ...
    let history = [];
    let length = (await contract.historyLength(account).call()).toNumber();
